...
(define (alphatize e)
  ...
  (define (rename-define def)
    (match def [`(define (,fname ,params ...) ,body)
		`(define (,fname ,@params)
		   ,((alpha-rename (foldl (lambda (x h) (hash-set h x x)) (hash) params))
		     body))]))
  (map rename-define e))
...
(define (anf-convert program)
  (define (anf-convert-define def)
    (match def
	   [`(define ,sig ,body)
	    `(define ,sig ,(normalize body (lambda (x) x)))]))

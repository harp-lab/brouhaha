...
; (define our-call
;   `((define (prov (define (interleave-direct lst1 lst2)
;                   (if (null? lst1)
;                       lst2
;                       (if (null? lst2)
;                           lst1
;                           (cons (car lst1)
;                                 (cons (car lst2) (interleave-direct (cdr lst1) (cdr lst2))))))))
;     (interleave-direct lst1 lst2)
;     (if-mark
;      (prov (if (null? lst1)
;                lst2
;                (if (null? lst2)
...
;      (app-mark (prov (null? lst1)) (atomic-mark (prov null?) null?) (atomic-mark (prov lst1) lst1))
;      (atomic-mark (prov lst2) lst2)
;      (add-prov-exp fb)))
;   (define (prov (define (brouhaha_main)
;                   (interleave-direct (list 1 2 3 4 5 6) (list 7 8 9 10))))
;     (brouhaha_main)
;     (app-mark (prov (interleave-direct (list 1 2 3 4 5 6) (list 7 8 9 10)))
;               (atomic-mark (prov interleave-direct) interleave-direct)
;               (app-mark (prov (list 1 2 3 4 5 6))
;                         (atomic-mark (prov list) list)
;                         (atomic-mark (prov 1) 1)
;                         (atomic-mark (prov 2) 2)
;                         (atomic-mark (prov 3) 3)
;                         (atomic-mark (prov 4) 4)
;                         (atomic-mark (prov 5) 5)
;                         (atomic-mark (prov 6) 6))
;               (app-mark (prov (list 7 8 9 10))
;                         (atomic-mark (prov list) list)
;                         (atomic-mark (prov 7) 7)
;                         (atomic-mark (prov 8) 8)
;                         (atomic-mark (prov 9) 9)
;                         (atomic-mark (prov 10) 10))))))

; (define our-call
;   `((define (prov (define (call num1 num2)
;                     (let ([x num1] [y num2]) x)))
;       (call num1 num2)
;       (let-mark (prov (let ([x num1] [y num2]) x))
;                 (((atomic-mark (prov x) x) (atomic-mark (prov num1) num1))
;                  ((atomic-mark (prov y) y) (atomic-mark (prov num2) num2)))
;                 (atomic-mark (prov x) x)))
;     (define (prov (define (brouhaha_main)
;                     (call 5 42)))
;       (brouhaha_main)
;       (app-mark (prov (call 5 42))
;                 (atomic-mark (prov call) call)
;                 (atomic-mark (prov 5) 5)
;                 (atomic-mark (prov 42) 42)))))
...
;     (let (prov
;           (let ((a2335 5))
;             (let ((a2336 42)) (app (prov (call 5 42)) call a2335 a2336))))
;       ((x2337
;         (let ((a2335 5))
;           (let ((a2336 42)) (app (prov (call 5 42)) call a2335 a2336)))))
;       x2337))))
...
; (define our-call
;   `((define (call kont2336 num1 num2)
;     (let kont2336 (prov (let ((x num1) (y num2)) x))
;       ((x num1))
;       (let (prov (let ((x num1) (y num2)) x)) ((y num2)) x)))
;   (define (brouhaha_main kont2337)
;     (let kont2337 (prov
;                    (let ((a2335 5))
;                      (let ((a2336 42))
;                        (app (prov (call 5 42)) call a2335 a2336))))
;       ((x2337
;         (let ((a2335 5))
;           (let ((a2336 42)) (app (prov (call 5 42)) call a2335 a2336)))))
;       x2337))))
...
(pretty-print (alphatize (cps-convert (anf-convert (add-tags (alphatize (desugar our-call)))))))
; (pretty-print (compile-to-finish our-call))

...
//    if ((v & 0x18) != FLOAT_VAL) // Ensure the mask and comparison is correct for a float.
//        assert_type(false, "Error in decode_float -> Type error: Not a Float!"); // Corrected error message.
//
//    // Assuming the float is stored in the higher 32-bits of the 64-bit integer.
//    uint32_t asU32 = (uint32_t)(v >> 32); // Cast to uint32_t to ensure correct bits are taken.
//
//    // Correctly interpret the bits as a float.
//    float decodedFloat = *reinterpret_cast<float*>(&asU32); // Correct the variable used for reinterpretation.
//
//    return decodedFloat;
//}
